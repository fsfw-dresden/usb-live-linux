...
grub_install() {
    if [ ! -d ${TMPDIR}/${LABEL_LIVE}/boot/grub ]; then
        ...
        grub-install --target=i386-efi --recheck --no-nvram --force-extra-removable --efi-directory=${TMPDIR}/${LABEL_WINDOWS_DATEN} --root-directory=${TMPDIR}/${LABEL_WINDOWS_DATEN}
        grub-install --target=x86_64-efi --recheck --no-nvram --force-extra-removable --efi-directory=${TMPDIR}/${LABEL_WINDOWS_DATEN} --root-directory=${TMPDIR}/${LABEL_WINDOWS_DATEN}
    else
